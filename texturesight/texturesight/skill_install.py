...
def default_skill_dir() -> Path:
    return Path.home() / ".claude" / "skills" / "texturesight"


def _source() -> Path:
    return Path(str(resources.files("texturesight") / "skill_data"))
...
def install_skill(target: Path | None = None, quiet: bool = False) -> Path:
    ...
    if not quiet:
        print(f"texturesight skill v{__version__} installed at {dst}")
    return dst
...
def _drop_umbrella() -> int:
    ...
    if not _installed("aisight"):
        return 0
    print("also removing the aisight umbrella (it requires texturesight) — "
...
def uninstall(remove_package: bool = True) -> int:
    ...
    if remove_package:
        print("removing the texturesight package...")
...
def maybe_autoinstall() -> None:
    ...
    try:
        ...
        print(("texturesight: Claude Code skill installed at "
               if fresh else
               "texturesight: Claude Code skill updated at ") + str(dst),
              file=sys.stderr)
    except Exception:
        pass
